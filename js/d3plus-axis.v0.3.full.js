...
{"width": 180, "height": 24, "x": 10, "y": 20}
  ...
  */
  Axis.prototype.render = function render (callback) {
    ...
    var tickScale = sqrt().domain([10, 400]).range([10, this._gridSize === 0 ? 32.5 : 75]);
    var labelScale = sqrt().domain([10, 400]).range([10, 75]);
    ...
    var textData = labels.map(function (d, i) {
      ...
      res.d = d;
      res.fS = s;
      res.width = Math.ceil(d3Max(res.lines.map(function (t) { return measure(t, {"font-family": f, "font-size": s}); }))) + s / 4;
    ...
    if (this._scale !== "band" && textData.length) {
      ...
      var firstB = d3Min([this._d3Scale(first.d) - first[width] / 2, range[0] - wBuff]);
      if (firstB < range[0]) {
      ...
      var lastB = d3Max([this._d3Scale(last.d) + last[width] / 2, range[lastI] + wBuff]);
    ...
    var obj;
    this._outerBounds = ( obj = {}, obj[height] = (d3Max(textData, function (t) { return t[height]; }) || 0) + (textData.length ? p : 0), obj[width] = rangeOuter[lastI] - rangeOuter[0], obj[x] = rangeOuter[0], obj );
