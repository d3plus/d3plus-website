...
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3-selection'), require('d3-transition'), require('d3plus-common'), require('d3-array'), require('d3-color'), require('d3plus-color'), require('d3plus-text'), require('d3-collection'), require('d3-interpolate-path'), require('d3-shape'), require('d3-polygon')) :
	typeof define === 'function' && define.amd ? define('d3plus-shape', ['exports', 'd3-selection', 'd3-transition', 'd3plus-common', 'd3-array', 'd3-color', 'd3plus-color', 'd3plus-text', 'd3-collection', 'd3-interpolate-path', 'd3-shape', 'd3-polygon'], factory) :
	(factory((global.d3plus = global.d3plus || {}),global.d3Selection,global.d3Transition,global.d3plusCommon,global.d3Array,global.d3Color,global.d3plusColor,global.d3plusText,global.d3Collection,global.d3InterpolatePath,global.paths,global.d3Polygon));
}(this, (function (exports,d3Selection,d3Transition,d3plusCommon,d3Array,d3Color,d3plusColor,d3plusText,d3Collection,d3InterpolatePath,paths,d3Polygon) { 'use strict';
...
var Shape = (function (BaseClass$$1) {
  function Shape(tagName) {
    ...
    this._activeStyle = {
      "stroke": function (d, i) { return d3Color.color(this$1._stroke(d, i)).darker(2); },
    ...
    this._fontColor = function (d, i) { return d3plusColor.colorContrast(this$1._fill(d, i)); };
    ...
    this._labelRotate = d3plusCommon.constant(0);
  ...
  */
  Shape.prototype._applyActive = function _applyActive (elem$$1) {
    ...
    */
    ...
    elem$$1.transition().duration(0).call(d3plusCommon.attrize, activeStyle);
...
function(d, i, shape) {
  ...
  */
  Shape.prototype.labelRotate = function labelRotate (_) {
    return arguments.length
         ? (this._labelRotate = typeof _ === "function" ? _ : d3plusCommon.constant(_), this)
         : this._labelRotate;
  };
      ...
      for (var i$1 = 0; i$1 < modifOrigins.length; i$1++) {
        ...
        for (var a = 0; a < aRatios.length; a++) {
          ...
          while (right - left >= widthStep) {
            ...
            if (options.events) { events.push({type: "rectangle", areaFraction: width * height / area$$1, cx: cx, cy: cy, width: width, height: height, angle: angle, insidePoly: insidePoly}); }
...
var Area = (function (Shape$$1) {
  ...
  */
  Area.prototype._aes = function _aes (d) {
    ...
    var points = values.map(function (v, z) { return [this$1._x0(v, z), this$1._y0(v, z)]; })
      .concat(values.reverse().map(function (v, z) { return this$1._y1 ? [this$1._x(v, z), this$1._y1(v, z)] : [this$1._x1(v, z), this$1._y(v, z)]; }));
...
var shapeEdgePoint = function (angle, distance, shape) {
  ...
  if (angle < 0) { angle = pi$1 * 2 + angle; }

  if (shape === "square") {

    var diagonal = 45 * (pi$1 / 180);
    var x = 0, y = 0;
    ...
    if (angle < pi$1 / 2) {
      ...
      var tan$1 = Math.tan(pi$1 - angle);
      x -= angle < pi$1 - diagonal ? distance / tan$1 : distance;
      y += angle < pi$1 - diagonal ? distance : tan$1 * distance;
    }
    else if (angle < diagonal + pi$1) {
      x -= distance;
      y -= Math.tan(angle - pi$1) * distance;
    }
    else if (angle < 3 * pi$1 / 2) {
      x -= distance / Math.tan(angle - pi$1);
      y -= distance;
    }
    else if (angle < 2 * pi$1 - diagonal) {
      x += distance / Math.tan(2 * pi$1 - angle);
      y -= distance;
    }
    else {
      x += distance;
      y -= Math.tan(2 * pi$1 - angle) * distance;
    }
