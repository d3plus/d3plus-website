    ...
    */
    Shape.prototype._renderLabels = function _renderLabels () {
      ...
      this._labelClass
        ...
        .config(d3plusCommon.configPrep.bind(this)(this._labelConfig))
        .render();
  ...
  */
  var Whisker = /*@__PURE__*/(function (BaseClass) {
    function Whisker() {
      ...
      this._endpoint = d3plusCommon.accessor("endpoint", "Rect");
      this._endpointConfig = {
        Circle: {
          r: d3plusCommon.accessor("r", 5)
        }
      };
      this._length = d3plusCommon.accessor("length", 25);
      this._lineConfig = {};
      this._orient = d3plusCommon.accessor("orient", "top");
      this._x = d3plusCommon.accessor("x", 0);
      this._y = d3plusCommon.accessor("y", 0);
    ...
    */
    Whisker.prototype.render = function render (callback) {
      ...
      this._line = new Line()
        .data(lineData)
        .select(d3plusCommon.elem("g.d3plus-Whisker", {parent: this._select}).node())
        .config(d3plusCommon.configPrep.bind(this)(this._lineConfig, "shape"))
        .render(callback);
      ...
      d3Collection.nest()
        ...
        .forEach(function (shapeData) {
          ...
          this$1._whiskerEndpoint.push(new shapes[shapeName]()
            ...
            .select(d3plusCommon.elem(("g.d3plus-Whisker-Endpoint-" + shapeName), {parent: this$1._select}).node())
            .config({
              height: function (d) { return d.orient === "top" || d.orient === "bottom" ? 5 : 20; },
              width: function (d) { return d.orient === "top" || d.orient === "bottom" ? 20 : 5; }
            })
            .config(d3plusCommon.configPrep.bind(this$1)(this$1._endpointConfig, "shape", shapeName))
            .render());
        });
  ...
  */
  var Box = /*@__PURE__*/(function (BaseClass) {
    function Box() {
      ...
      this._orient = d3plusCommon.accessor("orient", "vertical");
      this._outlier = d3plusCommon.accessor("outlier", "Circle");
      this._outlierConfig = {
        Circle: {
          r: d3plusCommon.accessor("r", 5)
        },
      ...
      this._rectWidth = d3plusCommon.constant(50);
      this._whiskerConfig = {};
      this._whiskerMode = ["tukey", "tukey"];
      this._x = d3plusCommon.accessor("x", 250);
      this._y = d3plusCommon.accessor("y", 250);
    ...
    */
    Box.prototype.render = function render () {
      ...
      var filteredData = d3Collection.nest()
        ...
        .map(function (d) {
          ...
          else if (mode[0] === "extent") { d.lowerLimit = d3Array.min(values); }
          else if (typeof mode[0] === "number") { d.lowerLimit = d3Array.quantile(values, mode[0]); }

          if (mode[1] === "tukey") {
            d.upperLimit = d.third + (d.third - d.first) * 1.5;
            if (d.upperLimit > d3Array.max(values)) { d.upperLimit = d3Array.max(values); }
          }
          else if (mode[1] === "extent") { d.upperLimit = d3Array.max(values); }
          else if (typeof mode[1] === "number") { d.upperLimit = d3Array.quantile(values, mode[1]); }
      ...
      this._box = new Rect()
        ...
        .select(d3plusCommon.elem("g.d3plus-Box", {parent: this._select}).node())
        .config(d3plusCommon.configPrep.bind(this)(this._rectConfig, "shape"))
        .render();
      ...
      this._median = new Rect()
        ...
        .select(d3plusCommon.elem("g.d3plus-Box-Median", {parent: this._select}).node())
        .config(d3plusCommon.configPrep.bind(this)(this._medianConfig, "shape"))
        .render();
      ...
      this._whisker = new Whisker()
        .data(whiskerData)
        .select(d3plusCommon.elem("g.d3plus-Box-Whisker", {parent: this._select}).node())
        .config(d3plusCommon.configPrep.bind(this)(this._whiskerConfig, "shape"))
        .render();
      ...
      d3Collection.nest()
        ...
        .forEach(function (shapeData) {
          ...
          this$1._whiskerEndpoint.push(new shapes$1[shapeName]()
            .data(shapeData.values)
            .select(d3plusCommon.elem(("g.d3plus-Box-Outlier-" + shapeName), {parent: this$1._select}).node())
            .config(d3plusCommon.configPrep.bind(this$1)(this$1._outlierConfig, "shape", shapeName))
            .render());
        });
