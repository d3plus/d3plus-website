        ...
        if (wrappable.length && availableHeight > wrappable[0].height * 2) {
          ...
          while (line <= 5) {
            ...
            var loop = function ( x ) {
              ...
              var res$1 = wrap().fontFamily(label.f).fontSize(label.s).lineHeight(label.lh).width(w).height(h)(label.sentence);
              if (!res$1.truncated) {
                textSpace -= label.width;
                label.width = Math.ceil(max(res$1.lines.map(function (t) { return measure(t, {"font-family": label.f, "font-size": label.s}); }))) + label.s;
